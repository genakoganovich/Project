...
def to_zomf_corridor(input_name, output_name, save_header, max_angle, corridor_width, min_width, max_width,
                     min_velocity):
    df = pd.read_csv(input_name, sep='\t', skiprows=13, usecols=np.arange(7), dtype=np.float64,
                     names=save_header.split())
    max_time = df.iloc[:, 3].max()
    ...
    def update_width(time):
        return min_width + (time / max_time) * (max_width - min_width)

    df.iloc[:, 6] = max_angle
    df.iloc[:, 5] = df.iloc[:, 3].apply(update_width)
    df.iloc[:, 4] = df.iloc[:, 3]
